...
nommathilde = configcortana.nommathilde ; nomchercher=data.reco ;console.log(); console.log("mathilde s'appel dans cortana : "+nommathilde)
...
console.log();console.log( networkInterfaces );

try{console.log();console.log(data.myIPretour,' IP Appelant dans cortana!');myIPretour=data.myIPretour}
catch(err){console.log("")}

data.dictation=data.reco ;console.log(); console.log(' Data recus avant traitement ',data);
...
query=reco ; query=query.toLowerCase() ; query=query.trim()
query=query.replace(new RegExp('œuf dur',"gi"),"oeuf dure")

console.log();console.log('phrase recu  dans cortana : '+query);
...
//return false
                     ...
                     // try{
						...
						console.log();console.log('on test pour vérifier si sarah a répondu à la phrase complète : '+url1)
						data.dictation=query
						request = require('request');

      function emulation(ladate,letemps,query,reco) {console.log();console.log('on verifie dans le log');console.log();
          request({ url : url1 }, function (err, response, body){
			//on attends l'ecriture du log
              ...
              if(fichier.indexOf('RECOGNIZED Speech:')>-1){console.log();console.log("émulation trouvée donc plugin actif on s'arrete la, bref fini en 1");callback();return false}; 
              console.log();console.log('pas d émulation trouvée')
...
envoie="";restant="";garbage="";nomplug	=""
	...
	function finemul(envoie,restant,garbage,nomplug){//fin car on connais
		  ...
		  //nom de sarah pour l'emul en v3
...
ladate=year+'-'+month+'-'+day;letemps='['+heures+':'+minutes+':'+secondes;//console.log(ladate);console.log(letemps)
// on emul pour voir
url1 = 'http://127.0.0.1:8888/?emulate='+nomappel+' '+queryrecherche;console.log();console.log('on test pour vérifier si sarah a répondu à ceci en 2 : '+url1)
request = require('request');
filePathfichier = path.resolve('%CD%', './bin/'+ladate+'.log').replace('\\%CD%', '');fs.writeFileSync(filePathfichier,'')


      function emulationphrase(ladate,letemps,queryrecherche,query) {console.log();console.log('on verifie dans le log')
          request({ url : url1 }, function (err, response, body){
			//on attends l'ecriture du log
              fs=require('fs')
  ...
  //  console.log(pos);console.log(lon);console.log('rrrrrrrrr')
              if(fichier.indexOf('RECOGNIZED Speech:')>-1){console.log("émulation trouvée donc plugin actif on s'arrete la");callback();return false}; 
              console.log(); console.log('pas d émulation trouvée')
                  match3(query,queryrecherche)
                  return false
...
return false
    ...
    if ( data.reco.search("Siri") >-1){ScribeSpeak("tu m'as appelé siri, et puis quoi encore, pourquoi pas cortana")}
callback();return false
}//fin try
