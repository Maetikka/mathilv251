...
function browserTalk(txt) {
	sse.emit('/scribe', {command: "saying", 'html': txt.html, 'timeout': txt.timeout, 'pause':txt.pause, 'myIPretour':txt.myIPretour })
sse1.emit('/scri', {command: "texteplugin", texteplugin: txt.html});
}

function texteplugin(txt) {
	sse1.emit('/scri', {command: "texteplugin", texteplugin: txt});
	sse.emit('/scribe', {command: "texteplugin", texteplugin: txt});
}
function activePlugin(txt) {
	sse.emit('/scribe', {command: "plugin", plugin: txt});
	 sse1.emit('/scri', {command: "plugin", plugin: txt});
}
function icone(txt,txt2) {
	sse1.emit('/scri', {command: "icone", icone: txt , 'myIPretour':txt2});
	sse.emit('/scribe', {command: "icone", icone: txt , 'myIPretour':txt2});
}
function alarmeaudio(txt,txt1) {
	sse1.emit('/scri', {command: "alarmeaudio", alarmeaudio: txt , alarmeaudioimg: txt1});
	sse.emit('/scribe', {command: "alarmeaudio", alarmeaudio: txt , alarmeaudioimg: txt1});
}
function ImgAlarme(txt) {
	sse1.emit('/scri', {command: "ImgAlarme", ImgAlarme: txt});
	sse.emit('/scribe', {command: "ImgAlarme", ImgAlarme: txt});
}

//function visage(txt) {
//	sse1.emit('/scri', {command: "visage", visage: txt});
//}

function radio(txt,txt2) {
	sse.emit('/scribe', {command: "radio", radio: txt, 'myIPretour':txt2});
}

function musique(txt,txt2) {
	sse.emit('/scribe', {command: "musique", musique: txt, 'myIPretour':txt2});
}

function Deezer(txt,txt2) {
	sse.emit('/scribe', {command: "Deezer", Deezer: txt, 'myIPretour':txt2});
}

function geolocalisation(txt,txt2) {
	sse.emit('/scribe', {command: "geolocalisation", geolocalisation: txt, 'myIPretour':txt2});
}

function INFOMODIALE(txt) {
	sse.emit('/scribe', {command: "INFOMODIALE", INFOMODIALE: txt});
}

function SCENARIO(txt) {
	sse.emit('/scribe', {command: "SCENARIO", SCENARIO: txt});
}
...
function microMute(mute, callback, force) {
	console.log("MICRO ON:" + micro_ON+mute);
	microcoupe=mute
...
function startAskMe(question, reponses, options,callback) {
	SarahEcoute(false, function() {coupeask="on"
...
function _ScribeSpeak(tts, callback, SARAH_local) {
	...
	try{
		console.log("pieces interphone.................................."+PiecesInterphone)
		if(PiecesInterphone!==""){myIPretour=PiecesInterphone}
	}
	catch(err){console.log("pas              de                retour         intephone")}
...
try{console.log(alarmeaudiospeak,":  alarme  et vocalisation : ",tts);
	if(alarmeaudiospeak=='oncc'){return false}
}
catch(err){}
//	console.log("===================================");
//	console.log("TTS: " + tts);
...
//	console.log("SARAH_speak_queue: " + tts_queue.length);
	...
	browserAbort();
	microOFF(function() {//SCRIBE.visage("icones/Images/anim.gif");
		//browserStart();
		talk();
	});		
...
function talk () {
	...
	var url = sarahConfig.remote + "?tts=" +  "&sync=true"; //(async==true ? "&sync=true" : "");

	console.log("UR***************L: " + encodeURI(tts));
...
// var process = '"C:/Program Files/VideoLAN/VLC/vlc.exe " --fullscreen --one-instance --repeat --no-audio --no-osd --no-video-title-show "plugins/holo/holoparle.mp4"'; 
 // var child = exec(process);
	...
	request(url,	function (err, response, body){
		...
		//setTimeout(function(){ 
			microON(function() {//SCRIBE.visage("icones/Images/0.png");
				// rien ?
...
console.log(data.confidence,' IP appelant')
	try{if(mute=='off'){callback();return false;
		console.log(mute,' option mute')}
	}
	catch(err){}		
if(data.reco.search("géolocalisationnepastoucher")>-1){

SARAH.run('geolocalisation', { 'geolocalisation' : data.reco, 'myIPretour' : data.confidence});reponse="";callback();return false

	//data.reco=nommathilde+data.reco

}


try{
	//console.log(coupeask,silencedefinitif,"**********************************************************************",microcoupe)
	//if (microcoupe==1){callback();return false}

	if (( data.reco.search(nommathilde,"gi") >-1)|| (data.reco.search("Siri")>-1)|| (data.reco.search(nommathilde.toLowerCase(),"gi")>-1)||((silencedefinitif=="on")&&(coupeask=="off")) ){
		if(silencedefinitif=="on")	{data.reco=nommathilde+" "+data.reco}
SARAH.context.scribe.lastReco = data.reco;//autorun_browser(kill)
			...
			var exec = require('child_process').exec;
				...
				var proc114=__nircmd + 'win trans ititle "Google Chrome" 1'
				//console.log(visuel+'2')
				

				if(visuel!=="on"){//chrome caché
				...
				//var pro=__nircmd + 'win trans ititle "Google Chrome" 1' ; var ch=exec(pro)
					...
					//exec(proc111);
					//setTimeout(function(){exec(proc113); }, 500);
					//setTimeout(function(){exec(proc112); }, 1000);
					//setTimeout(function(){exec(proc114); }, 1500);
//exec(__nircmd + 'win hide ititle "chrome"')
//var exeinvisible=
exec(__nircmd + 'win min ititle "Google Chrome"')
				 }
				 else{//==on
				 	//exec(proc111)
				 	//setTimeout(function(){exec(proc113); }, 2000);
				

				 }

 SARAH.run('cortana', { 'reco' : data.reco, 'myIPretour' : data.confidence});reponse="";callback({'tts' : " "})		
	}//fin if 


else{//if (microcoupe==1){callback();return false}
myIPretour=data.confidence
console.log(myIPretour+" ip retour export action")
...
//console.log(Config);
path = require('path');
//SARAH.chromeless("http://localhost:8090", 80,1500,1050,0,0);
...
SARAH.context.scribe = {
	...
	'INFOMODIALE' : INFOMODIALE,
	'SCENARIO' : SCENARIO,
	'icone': icone,
	'texteplugin': texteplugin,
...
console.log("Préparation du Serveur HTTP sur le port " + port_https);
var app = express();
var app1 = express();
var appmusique = express();
// par défaut les pages sont dans STATIC
app.use(express.static(__dirname  + '/static'));
app1.use(express.static(__dirname  + '/static1'));
...
var SSE1 = require('sse-emitter');
sse1 = new SSE1({
  keepAlive: 40000, // in ms, defaults to 10000
});
var SSEMUSIQUE = require('sse-emitter');
ssemusique = new SSEMUSIQUE({
  keepAlive: 50000, // in ms, defaults to 10000
});
// la page de dialogue entre le scribe et Chrome
app.get('/scribe', sse.bind());
app1.get('/scri', sse1.bind());
app1.listen(8090);
...
console.log("Serveur HTTP en écoute sur le port " + port_https);
...
function autorun_browser(kill) {
	...
	//var proc1 = 'start chrome --new-window '+path.resolve('%CD%', './plugins/scribe/static/tutomathilde.html').replace('\\%CD%', ''); http://localhost:8090/
var proc = 'start chrome --no-borders --no-tabs --chrome-frame --window-size=800,600  -one-instance --reload --new-window "https://127.0.0.1:4300 " '
var proc = proc+path.resolve('%CD%', './plugins/scribe/Listing-plugins-SARAH/Listingplugins.html').replace('\\%CD%', '')
